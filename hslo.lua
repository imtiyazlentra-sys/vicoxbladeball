...
AutoParry.SpamService = function(self)
    local ball = AutoParry.GetBall()
    ...
    local targetDistance = LocalPlayer:DistanceFromCharacter(targetPos)

    local movementFactor = 1
    local moveDir = LocalPlayer.Character.Humanoid.MoveDirection
    local targetDir = (targetPos - LocalPlayer.Character.PrimaryPart.Position).Unit
    local targetMoveDir = closest.Humanoid.MoveDirection

    -- Deteksi kontak dekat
    local now = tick()
    local lastCloseContact = getgenv().LastCloseContact or 0
    local inCloseContact = getgenv().InCloseContact or false

    if targetDistance <= 3 then
        inCloseContact = true
    end

    if inCloseContact and targetDistance > 3.3 then
        inCloseContact = false
        lastCloseContact = now
    end

    local canUseDiv10 = (not inCloseContact) and ((now - lastCloseContact) >= 1.5)
    if canUseDiv10 and moveDir.Magnitude > 0.2 and moveDir:Dot(targetDir) < -0.4 then
        movementFactor = 10
    end
    if canUseDiv10 and targetMoveDir.Magnitude > 0.2 and targetMoveDir:Dot(-targetDir) < -0.4 then
        movementFactor = 10
    end

    local ping = Player.Entity.properties.ping or 0
    local maxSpamDistance = ping * 0.7 + math.min(speed / (movementFactor * 1.2), 80)
    ...
    if entityProps.Distance > maxSpamDistance or
       ballProps.Distance > maxSpamDistance or
       targetDistance > maxSpamDistance then
        return 0
    end

    local dotReduction = math.clamp(-dot, 0, 1)
    local dotImpact = math.clamp(dotReduction * (speed / 40), 0, 4)
    local spamAccuracy = maxSpamDistance - dotImpact

    -- Simpan state global
    getgenv().LastCloseContact = lastCloseContact
    getgenv().InCloseContact = inCloseContact
...
local parriedBalls = {}
  ...
  local autoSpamModule = MainTab:create_module({
    title = "Auto Spam",
    flag = "autoSpamModule",
    description = "Automatically spams parry when targeted",
    section = "right",
    callback = function(v)
        if v then
            Connections["autoSpam"] = RunService.PreSimulation:Connect(function()
                ...
                local ballTarget = ball:GetAttribute("target")
                if not ballTarget or ballTarget ~= tostring(LocalPlayer) then
                ...
                local distance = (LocalPlayer.Character.PrimaryPart.Position - ball.Position).Magnitude
                local closest = AutoParry.ClosestPlayer()
                local targetDist = closest and LocalPlayer:DistanceFromCharacter(closest.PrimaryPart.Position) or math.huge

                if spamAccuracy > 0 and distance <= spamAccuracy and targetDist <= spamAccuracy and Parries > (getgenv().ParryThreshold or 2.5) then
                    IsAutoSpamming = true
